...
import { MapPin, Calendar, Clock, Car, User, Phone, Mail, FileText, CheckCircle2, ShieldCheck, ArrowRight } from 'lucide-react';
    ...
    <div className="pt-28 pb-20 bg-dark-bg min-h-screen">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10">
          <span className="text-xs font-bold text-gold uppercase tracking-widest">Instant Online Reservation</span>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Fill out the details below for guaranteed vehicle allocation and instant booking reference.
          </p>
        </div>
        ...
        {successBooking ? (
          <motion.div
            ...
            className="glass-card p-8 sm:p-12 rounded-3xl border border-gold/40 text-center space-y-6 shadow-2xl"
          >
            <div className="w-20 h-20 bg-gold/20 text-gold rounded-full flex items-center justify-center mx-auto border border-gold/50">
              <CheckCircle2 className="w-12 h-12" />
            </div>

            <h2 className="font-serif text-3xl font-bold text-white">Booking Confirmed!</h2>
            <p className="text-sm text-gray-300">
              Thank you, <strong className="text-gold">{successBooking.customer_name}</strong>. Your trip reservation has been recorded successfully.
            </p>

            <div className="bg-dark-card p-6 rounded-2xl border border-gray-800 text-left space-y-3 max-w-md mx-auto text-sm">
            ...
            <p className="text-xs text-gray-400">
              Our dispatch team will contact you on <strong className="text-white">{successBooking.phone}</strong> shortly to confirm driver details.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="gold-btn px-6 py-3 rounded-xl text-xs font-bold uppercase"
              >
          ...
          <motion.form
            ...
            className="glass-card p-6 sm:p-10 rounded-3xl border border-gold/30 space-y-6 shadow-2xl"
          >
            {errorMsg && (
              <div className="bg-red-950/80 border border-red-500/50 text-red-200 text-sm p-4 rounded-xl">
                {errorMsg}
              </div>
            )}
            ...
            {/* Service & Route Info */}
            <div>
              <h3 className="font-serif text-lg font-bold text-gold mb-4 border-b border-gray-800 pb-2 flex items-center gap-2">
                <MapPin className="w-5 h-5 text-gold" /> 1. Trip & Location Details
              </h3>
            ...
            <div>
              <h3 className="font-serif text-lg font-bold text-gold mb-4 border-b border-gray-800 pb-2 flex items-center gap-2">
                <Car className="w-5 h-5 text-gold" /> 2. Schedule & Vehicle Selection
              </h3>
            ...
            <div>
              <h3 className="font-serif text-lg font-bold text-gold mb-4 border-b border-gray-800 pb-2 flex items-center gap-2">
                <User className="w-5 h-5 text-gold" /> 3. Customer Contact Details
              </h3>
            ...
            <div className="pt-6 border-t border-gray-800 flex items-center justify-between flex-wrap gap-4">
              <div>
                <span className="text-xs text-gray-400">Estimated Total Fare</span>
                <p className="font-serif text-3xl font-bold text-gold">₹{calculateEstimate().toLocaleString('en-IN')}</p>
              </div>

              <button
                type="submit"
                disabled={loading}
                className="gold-btn px-8 py-4 rounded-xl text-sm font-bold uppercase tracking-wider shadow-xl flex items-center gap-2 disabled:opacity-50"
              >
                {loading ? 'Submitting Reservation...' : 'Confirm & Reserve Cab'} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
