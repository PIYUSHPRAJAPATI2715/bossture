  ...
  return (
    <div className="pt-28 pb-20 bg-dark-bg min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="text-center py-20 text-gold text-lg">Loading routes...</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredRoutes.map((route) => (
              <motion.div
                key={route.id}
                whileHover={{ y: -4 }}
                className="glass-card p-6 rounded-2xl border border-gold/20 flex flex-col justify-between"
              >
                <div>
